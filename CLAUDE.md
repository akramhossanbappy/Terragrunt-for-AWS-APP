...
- The root `projects/live/production/terragrunt.hcl` generates the S3 backend (per-unit state key `production/<region>/<unit>/terraform.tfstate` in `singleapp-fifa-terraform-state-bucket-production`). Each region root then generates the correct AWS provider for services under that region. `waf-cloudfront` is deployed from `us-east-1`.
