...
Provisions the production per-microservice CI/CD stack, wrapped by
`modules/delivery`:
...
This unit is **not** what deploys this Terraform/Terragrunt repo —
that pipeline is bootstrapped separately by
`cicd-terraform (apply-from-local)/` at the true repo root (see
`../../../../CLAUDE.md`).
...
```bash
cd projects/live/production/ap-southeast-1/04-delivery
terragrunt init
terragrunt validate
terragrunt plan
terragrunt apply
