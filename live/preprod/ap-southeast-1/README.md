# preprod / ap-southeast-1 region

Region root for the preprod environment in AWS `ap-southeast-1`. **No
...
match the preprod sibling layout).

`modules/cicd` (used by `04-delivery`) wires a parallel preprod
environment path; the unit's `cls_environment` variable handles the
cross-env wiring independently.

## See also

- [`../../README.md`](../../README.md) — top-level pattern.
- [`../README.md`](../README.md) — preprod env overview, pre-flight.
