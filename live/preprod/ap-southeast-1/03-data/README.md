...
Provisions the dev data plane, wrapped by `modules/data`:
...
`kibana-alb`'s Terraform code looks up OpenSearch's ENIs by filtering
`data.aws_network_interfaces` on `requester-id = "amazon-elasticsearch"`.
...
`inputs = {}` block — they're injected as `TF_VAR_*` env vars by the
...
```bash
cd projects/live/dev/ap-southeast-1/03-data
terragrunt init
terragrunt validate
terragrunt plan
terragrunt apply
